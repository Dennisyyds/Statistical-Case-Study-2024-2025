...
for (topic in 1:length(HumanCorpus$authornames)){
  ...
  trainhumanfeatures <- HumanCorpus$features[-topic] #select the essays on this particular topic 
  ...
  numHuman<- nrow(Human_combined)
  truth <- c(rep(1, nrow(testhumanfeatures)), rep(2, nrow(testGPTfeatures)))
  Dispredictions <- NULL
  KNNpredictions <- NULL
  Ranpredictions <- NULL
  SVMpredictions <- NULL
